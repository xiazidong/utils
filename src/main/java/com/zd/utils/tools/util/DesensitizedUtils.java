...
import com.zd.utils.annotation.Desensitized;
import com.zd.utils.tools.util.bean.ObjectUtil;
    ...
    public static String getJson(Object javaBean) {
        ...
        if (null != javaBean) {
            try {
                ...
                Object clone = ObjectUtil.deepClone(javaBean);

                /* 定义一个计数器，用于避免重复循环自定义对象类型的字段 */
                Set<Integer> referenceCounter = new HashSet<Integer>();

                /* 对克隆实体进行脱敏操作 */
                DesensitizedUtils.replace(ObjectUtil.getAllFields(clone), clone, referenceCounter);
                    ...
                    if (null != value) {
                        ...
                        if (type.isArray()) {
                            ...
                            for (int i = 0; i < len; i++) {
                                ...
                                if (isNotGeneralType(arrayObject.getClass(), arrayObject, referenceCounter)) {
                                    replace(ObjectUtil.getAllFields(arrayObject), arrayObject, referenceCounter);
                                }
                            }
                            ...
                            while (it.hasNext()) {// TODO: 待优化
                                ...
                                if (isNotGeneralType(collectionObj.getClass(), collectionObj, referenceCounter)) {
                                    replace(ObjectUtil.getAllFields(collectionObj), collectionObj, referenceCounter);
                                }
                            }
                            ...
                            for (Object o : set) {
                                ...
                                if (isNotGeneralType(mapVal.getClass(), mapVal, referenceCounter)) {
                                    replace(ObjectUtil.getAllFields(mapVal), mapVal, referenceCounter);
                                }
                            }
                        ...
                        /*除基础类型、jdk类型的字段之外，对其他类型的字段进行递归过滤*/
                            ...
                            if (!type.isPrimitive()
                                ...
                                replace(ObjectUtil.getAllFields(value), value, referenceCounter);
                            }
                        }
                    }
                    //脱敏操作
